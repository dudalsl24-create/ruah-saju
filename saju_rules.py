# saju_rules.py — 외부 CSV 업로드 없이 동작
# - sajupy 패키지에 내장된 manse_utf8.csv를 자동 로딩
# - 음력 입력 지원(음→양 매핑)
# - 자시/조자시 (+30분) 보정, 시두법 반영
...
MONTH_BOUNDARIES = [  # 로컬 CSV 없을 때만 백업용으로 사용
...
def _load_manse_db():
    """우선순위: ①프로젝트 data/ ②sajupy 패키지 내장 CSV"""
...
def get_pillars(y:int,m:int,d:int, hh:int,mm:int, is_lunar:bool=False)->Pillars:
    """DB가 있으면 DB 기준(연/월/일), 없으면 백업 규칙 사용"""
    _load_manse_db()
    ...
    if is_lunar and _DB_LUNAR:
        key=f"{y}-{m}-{d}-L"
        if key in _DB_LUNAR:
            t=_DB_LUNAR[key]; y,m,d = t["y"], t["m"], t["d"]
